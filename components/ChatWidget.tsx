    ...
    <>
      <div className={`fixed bottom-8 right-8 z-[100] ${isOpen ? 'hidden' : 'block'}`}>
        <button
          ...
          className="relative group flex items-center justify-center w-16 h-16 bg-primary text-white rounded-full shadow-2xl shadow-primary/30 transition-all hover:scale-110 active:scale-95 overflow-hidden"
        >
          <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/20 to-transparent animate-pulse-slow"></div>
          <MessageCircle size={32} />

          <span className="absolute right-20 bg-surface text-white text-xs py-2 px-4 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none border border-white/10 shadow-xl">
            {t.chat.tooltip}
          </span>
        </button>
      </div>
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-[450px] glass-panel z-[100] shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex items-center justify-between p-6 border-b border-white/10 bg-surface/50">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  <h3 className="text-white text-lg font-bold leading-none">{t.chat.aiName}</h3>
                  <p className="text-slate-400 text-xs font-normal mt-1">{t.chat.aiRole}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setIsVoiceEnabled(!isVoiceEnabled)}
                  className={`p-2 rounded-full transition-colors ${isVoiceEnabled ? 'text-primary bg-primary/10' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 scrollbar-thin">
              <div className="flex justify-center">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-white/5 px-3 py-1 rounded-full">
                  Powered by Gemini 3 • Professional Translation
                </span>
              </div>
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex items-start gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  {msg.role === 'model' ? (
                    ...
                    <div className="bg-slate-700 flex items-center justify-center rounded-full size-8 shrink-0 text-white mt-1">
                      <span className="text-xs font-bold uppercase">{language === 'zh' ? '您' : 'You'}</span>
                    </div>
                  )}

                  <div className={`flex flex-col gap-1 max-w-[85%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                    <p className="text-slate-400 text-[11px] font-medium leading-none mb-1">
                      {msg.role === 'model' ? 'Binh Assistant' : (language === 'zh' ? '访客' : 'You')}
                    </p>
                    <div className={`text-sm font-normal leading-relaxed px-4 py-3 rounded-2xl whitespace-pre-wrap shadow-sm transition-all ${msg.role === 'user'
                        ...
                        : 'bg-surface text-slate-100 rounded-tl-sm border border-white/5'
                      }`}>
                      {msg.text}
                      {msg.isStreaming && (
                ...
                <div className="flex gap-2 flex-wrap ml-11">
                  {[
                    ...
                    <button
                      ...
                      className="flex h-8 shrink-0 items-center justify-center gap-x-2 rounded-lg bg-surface border border-white/10 px-3 hover:bg-white/5 hover:border-primary/40 transition-all group"
                    >
                      <span className="text-sm group-hover:scale-110 transition-transform">{chip.icon}</span>
                      <p className="text-slate-300 text-xs font-semibold">{chip.text}</p>
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="p-6 border-t border-white/10 bg-background/95">
              <div className="flex flex-col gap-3">
                <div className="relative flex items-center">
                  <textarea
                    ...
                    disabled={isTyping}
                    className="w-full bg-[#0d141b] border border-white/10 rounded-xl px-4 py-3 pr-12 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary resize-none min-h-[50px] max-h-[120px] disabled:opacity-50"
                ...
                <div className="flex justify-between items-center px-1">
                  <div className="flex gap-4">
                    ...
                    <button
                      ...
                      className={`transition-colors shadow-sm ${isListening ? 'text-primary scale-110' : 'text-slate-500 hover:text-white'}`}
                    >
                      {isListening ? <Mic size={18} /> : <MicOff size={18} />}
                    </button>
                  </div>
                  <p className="text-[10px] text-slate-600 font-medium">{language === 'zh' ? 'Shift + Enter 换行' : 'Shift + Enter for new line'}</p>
                </div>
              </div>
            </div>
